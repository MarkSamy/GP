				...
				if (localName.equals("node") || (localName.equals("way") && ((Way) current).getNodes().size() >= 2)
					...
					if (current instanceof Node) {
						...
						for (int i = 1; i < currentWay.getNodes().size(); i++) {
							String currentNode = currentWay.getNodes().get(i);
							String lasttNode = currentWay.getNodes().get(i-1);
							csvWaysBuild.append("," + currentNode);
							if(! (currentNode.compareTo(lasttNode) == 0) ){
								edgesWriter.println(currentNode + ";" + lasttNode);
								edgesWriter.println(lasttNode + ";" + currentNode);
								nbEdges+=2;
							}
						}
						csvWaysBuild.append("]\";");
